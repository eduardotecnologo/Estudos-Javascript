...
export default class AlbumMainDetails extends Component {
  ...
  renderAlbumSongs() {
    ...
    const artists = (details.artists && details.artists.length > 0) ?
      <li className="list-group-item"><b>Artista: </b>
        <span>{details.artists[0].name}</span></li> : null;

    const image = (details.images && details.images.length > 0) ? details.images[0].url : ALBUM_PLACEHOLDER;

    const name = (details.name) ? <li className="list-group-item"><b>Nome: </b>
        <span>{details.name}</span></li> : null;

    const genre = (details.genres && details.genres.length > 0) ?
      <li className="list-group-item"><b>Genero: </b>
        <span>{details.genres[0]}</span></li> : null;

    const popularity = details.popularity ?
      <li className="list-group-item"><b>Popular: </b>
        <span>{details.popularity}</span></li> : null;

    const releaseDate = details.release_date ?
      <li className="list-group-item"><b>Data de Lançamento: </b>
        <span>{details.release_date}</span></li> : null;

    const numberOfTracks = details.tracks ? <li className="list-group-item"><b>Faixas: </b>
        <span>{details.tracks.total}</span></li> : null;
